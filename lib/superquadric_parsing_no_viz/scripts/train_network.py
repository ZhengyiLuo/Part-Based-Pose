...
def main(argv):
    ...
    writer = SummaryWriter('/scratch/karls/event_logs/ziad')
    ...
    for i in range(args.epochs):
        ...
        for b, sample in zip(list(range(args.steps_per_epoch)), yield_infinite(train_bp)):
            ...
            if (print_to_stdout % 50) == 0:
                writer.add_scalar('train/loss', bar.loss, args.steps_per_epoch * i + print_to_stdout)
                percentage_epoch = float(print_to_stdout / float(args.steps_per_epoch)) * 100
                print("Epoch {0} ({1:05.2f}%): loss={2}, pcl_to_prim={3}, prim_to_pcl={4}".format(i, percentage_epoch, bar.loss, bar.pcl_to_prim_loss, bar.prim_to_pcl_loss))
            print_to_stdout += 1
