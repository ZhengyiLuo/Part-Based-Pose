...
def l2_loss(y_hat, y_target, device):
    y_hat = y_hat.to(device)
    y_target = y_target.to(device)
    loss = torch.norm(y_hat - y_target, dim=2)
    return loss
...
def matrix_loss(y_hat, y_target, options):
    ...
    poses_target = y_target.view(B, M, 7)
    # print(poses_target[0])
    # print("_______________________")
    rotations_target = poses_target[:, :, :4].view(B, M, 4)
    # print(rotations_target[0])
    # print("_______________________")
    translations_target = poses_target[:, :, 4:].view(B, M, 3)

    # print(translations_target[0])
    # print("_______________________")
    # exit(1)
    ...
    translations = y_hat[0].view(B, M, 3)
    rotations = y_hat[1].view(B, M, 4)
